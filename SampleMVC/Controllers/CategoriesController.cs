...
public class CategoriesController : Controller
{
    ...
    public async Task<IActionResult> ServicesPaging(int pageNumber = 1, int pageSize = 5, string search = "", string act = "")
    {
    ...
    [HttpPost]
    public async Task<IActionResult> InsertFromServices(CategoryCreateDTO categoryCreate)
    {
        try
        {
    ...
    [HttpPost]
    public async Task<IActionResult> EditFromServices(Category category)
    {
        try
        {
            var test = category.categoryID;
            await _categoryServices.Update(test, category);
        }
        catch (Exception ex)
        {
            ViewData["message"] = $"<div class='alert alert-danger'><strong>Error!</strong>{ex.Message}</div>";
